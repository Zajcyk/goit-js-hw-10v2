...
// Funkcja do pobierania danych o krajach z API Rest Countries v2
function fetchCountries(name) {
  var fields = 'name.official,capital,population,flags.svg,languages'; // Wymagane pola
  var url = "https://restcountries.com/v2/name/".concat(name, "?fields=").concat(fields); // URL API z parametrami

  // Wysłanie żądania HTTP do API
  return fetch(url).then(function (response) {
    if (!response.ok) {
      console.log("error");
      throw new Error(response.status); // Jeśli odpowiedź jest błędna, rzuć błąd
    }
...
//   });
// };
},{}],"node_modules/notiflix/dist/notiflix-aio-3.2.7.min.js":[function(require,module,exports) {
var define;
var global = arguments[3];
...
// Import funkcji do pobierania danych o krajach
// Import biblioteki Notiflix do wyświetlania powiadomień
...
// Dodanie event listenera do pola wyszukiwania z funkcją debounce
searchBox.addEventListener('input', (0, _lodash.default)(onSearch, DEBOUNCE_DELAY));

// Funkcja wywoływana przy wyszukiwaniu
function onSearch(event) {
  var searchQuery = event.target.value.trim(); // Pobranie i oczyszczenie wprowadzonego tekstu

  if (!searchQuery) {
    clearResults(); // Jeśli pole jest puste, wyczyść wyniki
    console.log("Clear result");
    return;
  }
  (0, _fetchCountries.fetchCountries)(searchQuery).then(function (countries) {
    clearResults(); // Wyczyść wyniki poprzedniego wyszukiwania

    if (countries.length > 10) {
      _notiflix.default.Notify.info('Too many matches found. Please enter a more specific name.');
    } else if (countries.length >= 2 && countries.length <= 10) {
      console.log("countries:", countries);
      renderCountryList(countries); // Wyświetl listę krajów
    } else if (countries.length === 1) {
      renderCountryInfo(countries[0]); // Wyświetl informacje o jednym kraju
      console.log("show one country");
    }
  }).catch(function (error) {
    clearResults(); // Wyczyść wyniki
    _notiflix.default.Notify.failure('Oops, there is no country with that name'); // Pokaż komunikat o błędzie
  });
}
...
function renderCountryList(countries) {
  console.log("renderCountryList function called");
  var markup = countries.map(function (country) {
    return "\n        <li>\n          <img src=\"".concat(country.flags.svg, "\" alt=\"Flag of ").concat(country.name.official, "\" width=\"50\" height=\"30\">\n          <p>").concat(country.name.official, "</p>\n        </li>");
  }).join('');
  console.log("markup:", markup);
  countryList.innerHTML = markup;
}

// Funkcja do renderowania informacji o jednym kraju
function renderCountryInfo(country) {
  var languages = Object.values(country.languages).join(', ');
  var markup = "\n    <div>\n      <img src=\"".concat(country.flags.svg, "\" alt=\"Flag of ").concat(country.name.official, "\" width=\"100\" height=\"60\">\n      <h2>").concat(country.name.official, "</h2>\n      <p><strong>Capital:</strong> ").concat(country.capital, "</p>\n      <p><strong>Population:</strong> ").concat(country.population, "</p>\n      <p><strong>Languages:</strong> ").concat(languages, "</p>\n    </div>\n  ");
  console.log(markup);
  countryInfo.innerHTML = markup;
}
},{"lodash.debounce":"node_modules/lodash.debounce/index.js","./fetchCountries":"js/fetchCountries.js","notiflix":"node_modules/notiflix/dist/notiflix-aio-3.2.7.min.js"}],"../../../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
var global = arguments[3];
